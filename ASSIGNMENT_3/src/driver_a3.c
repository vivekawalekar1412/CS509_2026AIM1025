...
void print_output(const char *alg_name, const MSTResult *res, int V) {
    printf("Algorithm: %s\n", alg_name);
    if (V <= 20) {
        printf("MST edges:\n");
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(mode, "kruskal") == 0) {
        ...
        print_output("Kruskal's MST", &k_res, csr->num_vertices);
        free_mst_result(&k_res);
    } else if (strcmp(mode, "prim") == 0) {
        MSTResult p_res = run_prim(csr);
        print_output("Prim's MST", &p_res, csr->num_vertices);
